...
NFerrorsnonfatal = [
    ...
    {'infile':'NF01.edi','errortext':'''MessageError: [F19] Record "[['UNH'], ['ALI']]" too many fields in record; unknown field "ER" (line 5 position 14). [F18] Record "[['UNH'], ['NAD']]" too many subfields in composite; unknown subfield "9" (line 12 position 11). [F17] Record "[['UNH'], ['NAD']]" expect field but "9" is a subfield (line 13 position 8). [S03] Record "[['UNH'], ['DTM']]" occurs 1 times, min is 2. [F06] Record "[['UNH'], ['ALI']]" field "4183#5" too small (min 3): "ER". [S04] Record "[['UNH'], ['IMD']]" occurs 2 times, max is 1. [F05] Record "[['UNH'], ['NAD']]" field "3035" too big (max 3): "SUFF". [F02] Record "[['UNH'], ['NAD']]" field "3035" is mandatory. [F04] Record "[['UNH'], ['NAD']]" subfield "C082.3039" is mandatory: "{'3035': u'BU', 'BOTSID': u'NAD', 'C082.3055': u'9', 'BOTSIDnr': u'1'}". [F09] Record "[['UNH'], ['LIN'], ['QTY']]" field "C186.6060" contains exponent: "E12". [F16] Record "[['UNH'], ['LIN'], ['QTY']]" numeric field "C186.6060" has non-numerical content: "E12". [F10] Record "[['UNH'], ['LIN'], ['QTY']]" field "C186.6060" too big (max 15): "1234567890123456789". [F16] Record "[['UNH'], ['LIN'], ['QTY']]" numeric field "C186.6060" has non-numerical content: "12JAJA". [F11] Record "[['UNH'], ['LIN'], ['QVR']]" field "C279.6064" too small (min 6): "123". [F03] Record "[['UNH'], ['LIN'], ['DOC']]" composite "C002" is mandatory.'''},
...
Aerrors = [
    {'infile':'A54_01.edi','errortext':'''InMessageError: [A54] Incoming edi file uses non-standard separators - should use UNA.'''},
    # zero content
    {'infile':'A55_01.edi','errortext':'''InMessageError: [A55] Edi file only contains whitespace.'''},
    #~ #no UNB after UNZ
    {'infile':'A56_01.edi','errortext':'''InMessageError: [A56] Edi file contains data not in a valid interchange at position 489'''},
    {'infile':'A56_02.edi','errortext':'''InMessageError: [A56] Edi file contains data not in a valid interchange at position 246'''},
    {'infile':'A56_03.edi','errortext':'''InMessageError: [A56] Edi file contains data not in a valid interchange at position 246'''},
    {'infile':'A57_01.edi','errortext':'''InMessageError: [A57] Edi file has no valid content (does not start with a valid interchange).'''},
    #~ #no UNB in start of edifact file
    {'infile':'A57_02.edi','errortext':'''InMessageError: [A57] Edi file has no valid content (does not start with a valid interchange).'''},
    # other content
    {'infile':'A57_04.edi','errortext':'''InMessageError: [A57] Edi file has no valid content (does not start with a valid interchange).'''},
...
Eerrors = [
     ...
     {'infile':'E54_01.edi','errortext':'''InMessageError: [E54] Not allowed chars in incoming edi file at/after filepos: 63'''},
     {'infile':'E54_02.edi','errortext':'''InMessageError: [E54] Not allowed chars in incoming edi file at/after filepos: 63'''},
   ]
Serrors = [
    ...
    #~ #no BGM (M)
    {'infile':'S50_03.edi','errortext':'''InMessageError: [S50] Line:3 pos:1 record:"DTM": message has an error in its structure; this record is not allowed here. Scanned in message definition until mandatory record: "[['UNH'], ['BGM']]". 	'''},
